...
/* controlador para la subida de imagenes */
...
UserCtrl2.leerusuario = async (req, res) => {
  ...
  const usuarioUnico = await Guardarmodelo2.findById(
    {
      _id: identificador,
...
UserCtrl2.modificar = async (req, res) => {
  ...
  if (req.file) {
    ...
    const { nombre,descripcion,precio,estrella,correo, imagen, imageURL } =
      req.body; /* sacar los datos quese necesitan del req body */

    await Guardarmodelo2.findByIdAndUpdate(
      { _id: identificador },
      /* busca ese id en la base de datos comparando el _id
       */
      {
    ...
    res.json({
      mensaje:
        "Mensaje desde el Backend: modifica el usuario con el id Imagen Enviada",
    });
    /* SI EL ENVIO NO TIENE UN ARCHIVO IMAGEN */
  } else {
    const {  nombre,descripcion,precio,estrella,correo, imagen,imageURL } =
      req.body; /* sacar los datos quese necesitan del req body modifica el usuario con el id Sin Imagen*/
    await Guardarmodelo2.findByIdAndUpdate(
      { _id: identificador },
      /* busca ese id en la base de datos comparando el _id
       */
    ...
    res.json({
      mensaje:
        "Mensaje desde el Backend: modifica el usuario con el id Sin Imagen",
    });
  }
};
...
UserCtrl2.borrar = async (req, res) => {
  ...
  /* const {imagenURL}= await Guardarmodelo2.findById({ _id: identificador })
    ...
    console.log('++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    await cloudinary.v2.uploader.destroy(imagenURL) */
  const { imagenURL } = await Guardarmodelo2.findByIdAndDelete(
    {
      _id: identificador,
    } /* busca ese id en la base de datos comparando el _id  */,
    req.body
...
UserCtrl2.crear = async (req, res) => {
  ...
  const { nombre, descripcion, precio, estrella, correo, imagen, imageURL } =
    req.body; /* estos corresponde a los del models.js que llegan del frontend*/
  console.log(req.body);
  const resultado = await cloudinary.v2.uploader.upload(req.file.path);
  ...
  res.json({
    mensaje: "Mensaje desde el Backend: Producto guardado exitosamente",
  }); /* este mensaje se puede mostrar en el frontent */
