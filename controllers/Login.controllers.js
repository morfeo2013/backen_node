...
LogingUsuario.registrar = async (req, res) => {
  ...
  const { nombre, correo, contrasena } =
    req.body; /* se ecuentra en el req del body que secibio del frontend*/
  ...
  if (copiaModeloDeBacken) {
    ...
    const nuevoLogings = new Guardarmodelo1({
      nombre,
      correo,
      contrasena,
      admin: "",
    ...
    const token = jwt.sign(
      {
        _id: nuevoLogings._id /* si se quiere colocar otro elemento se agrea una coma y se especifica que propiedad se va a crear */,
      },
      "lol"
    ); /* esto crea una cabezera donde ira el token */
...
LogingUsuario.ingresar = async (req, res) => {
  ...
  if (copiaModeloDeBacken) {
    ...
    bcrypt.compare(
      contrasena,
      copiaModeloDeBacken.contrasena,
      function (err, resp) {
        if (resp) {
          /* generar el token */
          ...
          res.json({
            ...
            correo: copiaModeloDeBacken.correo,
...
await Guardarmodelo1.findByIdAndUpdate({ _id: copiaModeloDeBacken._id}, 
  ...
  if (copiaModeloDeBacken) {
    ...
    res.json({
      mensage:
        "Tu Contraseña a sido enviada a tu Correo desde el backend" +
        token /* +copiaModeloDeBacken.id+token */ /* +ip_info.clientIp+city+region */,
      /* estos datos se envian si la contraseña y la claves son correcta  al sesionStorange del fronnen */
    });
...
LogingUsuario.Password = async (req, res) => {
  const idUsuario =
    req.params
      .usuario; /* debe coicidir con loging.routers para recivir desde el frontend */
  const contrasena =
    req.params
      .password; /* debe coicidir con loging.routers para recivir desde el frontend */

  /* implementar la buaqueda en la basede datos */
  const nuevaContrasena = await Guardarmodelo1.findByIdAndUpdate(
    { _id: idUsuario },
    /* busca ese id en la base de datos comparando el _id
     */
    {
...
LogingUsuario.listarusuarios = async (req, res) => {
  ...
  const listarusuarios = await Guardarmodelo1.find(
    {} /* ,{contrasena:0} */
  ).populate("user2");
...
LogingUsuario.loginIdFavoritos = async (req, res) => {
  ...
  await Guardarmodelo1.updateOne(
...
LogingUsuario.leeridFavoritos = async (req, res) => {
  /* res.send('hola mundo')  */
  ...
  const usuarioUnicoFav = await Guardarmodelo1.findById(
    {
      _id: identificadorFav,
    } /* busca ese id en la base de datos comparando el _id  */,
    req.body
  ).populate(
    "user2"
  ); /* muestraun usuario index todo lo que le llegue por el req.bodyen el _id encontrado  con findById*/

  res.json(
    usuarioUnicoFav
  ); /* responde y envia los datos para ser leidos por el res y los datos por el usuarioUNICO */
};
...
LogingUsuario.loginId = async (req, res) => {
  /* el fin da la orden de tomar todos los datos de usuarios */
  const id =
    req.params.id; /* obtiene el id del uusario recivido desde frontend */
  const listarusuariosId = await Guardarmodelo1.findById(
    { _id: id } /* ,{contrasena:0} */
  ); /* compara el de la base de datos con el del frontend y envia el usario que tenga ese id*/
...
LogingUsuario.borrar = async (req, res) => {
  ...
  await Guardarmodelo1.findByIdAndDelete(
    {
      _id: identificador,
    } /* busca ese id en la base de datos comparando el _id  */,
    req.body
...
LogingUsuario.borrarFavoritos = async (req, res) => {
  ...
  const identificador =
    req.params
      .dir; /* debe coicidir con loging.routers para recivir desde el frontend */
  const identificador2 =
    req.params
      .producto; /* debe coicidir con loging.routers para recivir desde el frontend */
  /*   console.log(identificador2) */
  await Guardarmodelo1.findByIdAndUpdate(
    { _id: identificador },
    { $pull: { user2: identificador2 } }
  ); /* pull para borrar ese elemento dntro del otroelemento de la tabla sin afectar la tabla original */
  await Guardarmodelo1.updateOne(
    // aggregate significa que vamos a agrupar
    // lista de operaciones, a realizar en secuencia
    // en este caso solo una operación, agrupar
  ...
  res.json({
    mensaje:
      "Mensaje desde el Backend: usuario fue eliminado desde el delete FAVORITOS",
  });
};
